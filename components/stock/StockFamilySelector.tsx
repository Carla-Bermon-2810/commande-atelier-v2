...
export default function StockFamilySelector({ backHref, eyebrow, title, description, items }: Props) {
  return (
    <main className="mx-auto max-w-7xl py-4">
      ...
      <div className={`grid grid-cols-1 gap-4 sm:grid-cols-2 ${items.length > 3 ? "xl:grid-cols-4" : "lg:grid-cols-3"}`}>
        {items.map((item) => (
          <Link key={item.href} href={item.href} className="group overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-[#F95516] hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-[#F95516] focus:ring-offset-2">
